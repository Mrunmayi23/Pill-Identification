...
def image_classifier(input_img):
        ...
        #def extract_text(df_column):
        #    clean_text = df_column.str.replace(r'[^a-zA-Z0-9\s]', '', regex=True)  # Remove special characters
        #    clean_text = df_column.str.replace(r'\s(?=\d)|(?<=\d)\s', '', regex=True)   #Remove spaces
        #    return clean_text.str.join('')
        
        #clean_text = extract_text(df[:])
